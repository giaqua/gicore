/**
 * task_tracker.js - Optimized Space Design with Sidebar Updates
 */
...
frappe.pages["task-tracker"].on_page_load = function (wrapper) {
    ...
    const state = {
        ...
        loading: false,
        view_mode: "grid",
        current_task: null, // Track currently open task in sidebar
    };
    ...
    // ══════════════════════════════════════════════════════════════════════
    // Helper function to update task in sidebar
    // ══════════════════════════════════════════════════════════════════════
    function updateSidebarTask(task_name, field, value) {
        if (state.current_task === task_name) {
            ...
            if (field === "exp_end_date") {
                const due_str = value ? frappe.datetime.str_to_user(value) : "—";
                $panel.find(".tt-detail-row:contains('Due Date') span").text(due_str);
            }
            
            // Show alert in sidebar
            frappe.show_alert({ 
                message: `${field} updated to <b>${value}</b>`, 
                indicator: "green" 
            }, 2);
        }
    }
    ...
    function render_stat_bar(counts) {
        const items = [
            { label: "All", value: counts.total, cls: "tt-stat-total", icon: "fa-tasks" },
            { label: "Open", value: counts.open, cls: "tt-stat-open", icon: "fa-play-circle" },
            { label: "Working", value: counts.working, cls: "tt-stat-working", icon: "fa-cog" },
            { label: "Overdue", value: counts.overdue, cls: "tt-stat-overdue", icon: "fa-exclamation-triangle" },
            { label: "Completed", value: counts.completed, cls: "tt-stat-completed", icon: "fa-check-circle" },
        ];
        $("#tt-stats").html(
            items.map(i =>
                `<div class="tt-stat ${i.cls}" data-filter="${i.label === 'All' ? '' : i.label}">
    ...
    function render_tasks(tasks, state) {
        ...
        $list.find(".tt-status-select, .tt-status-select-sm").off("change").on("change", function() {
            ...
            frappe.call({
                ...
                callback: function() {
                    ...
                    // Update sidebar if this task is currently open
                    updateSidebarTask(task_name, "status", new_status);
                    
                    // Reload to refresh stats and views
                    load();
                },
            });
        });
        ...
        $list.find(".tt-icon-btn, .tt-icon-btn-sm").off("click").on("click", function(e) {
			...
			window.open(`/app/task/${$(this).data("task")}`, '_blank');
        });
    }
    ...
    // ══════════════════════════════════════════════════════════════════════
    // Quick-review slide panel with update capabilities
    // ══════════════════════════════════════════════════════════════════════
        ...
        frappe.call({
            ...
            callback: function(r) {
                ...
                $panel.find("#tt-panel-body").html(`
                    ...
                    <div class="tt-panel-details">
                        <div class="tt-detail-row">
                            <label>Project:</label>
                            <span>${escape_html(t.project || "—")}</span>
                        </div>
                        <div class="tt-detail-row">
                            <label>Task Type:</label>
                            <span>${escape_html(t.type || "—")}</span>
                        </div>
                        <div class="tt-detail-row">
                            <label>Due Date:</label>
                            <span>${t.exp_end_date || "—"}</span>
                        </div>
                        <div class="tt-detail-row">
                        ...
                        <div class="tt-detail-row">
                            ...
                            <span>${escape_html(t.parent_task || "—")}</span>
                        </div>
                        <div class="tt-detail-row">
                            <label>Assigned To:</label>
                            <span>${t.assignees && t.assignees.length ? t.assignees.join(", ") : "—"}</span>
                        </div>
                    </div>
                    ...
                    <!-- Inline status update in sidebar -->
                ...
                $panel.find(".tt-add-comment-btn").off("click").on("click", function() {
                    ...
                    frappe.call({
                        method: "gicore.gi_core.page.task_tracker.task_tracker.add_comment",
                        args: { task_name, comment: comment },
                        callback: function() {
                            frappe.show_alert({ message: "Comment added", indicator: "green" }, 2);
                            $textarea.val("");
                            // Refresh the panel to show new comment
    ...
    function open_add_dialog(state, reload) {
        const d = new frappe.ui.Dialog({
            ...
            fields: [
                ...
                { label: "Task Type", fieldname: "task_type", fieldtype: "Link", options: "Task Type", default: state.task_type || "" },
...
// HTML template (add priority filter)
const TT_HTML = `
<div id="tt-root">
