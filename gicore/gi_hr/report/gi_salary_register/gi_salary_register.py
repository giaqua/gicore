...
def execute(filters=None):
	...
	for ss in salary_slips:
		...
		for e in earning_types:
			salary_component_type = get_custom_salary_component_type(e)
			
			component_amount = ss_earning_map.get(ss.name, {}).get(e) if ss_earning_map.get(ss.name, {}).get(e) else 0
			if salary_component_type == "Basic":
				basic = basic +component_amount
				
			elif salary_component_type == "Housing Allowance":
				housing = housing + component_amount

			elif salary_component_type == "Other Earning":
				other_earning = other_earning+component_amount

		row.update({"basic_salary": basic})
		row.update({"housing_allowance": housing})
		row.update({"other_allowance": other_earning})
...
def get_custom_salary_component_type(salary_component):
	return frappe.db.get_value("Salary Component", salary_component, "custom_salary_component_type")
